---
name: corpus-cleaner
description: 把一堆混杂的文档资料包(PDF/PPTX/PPT/DOCX/DOC/epub,可能含扫描书和设计版面)清洗、提取、OCR、分类,归档成结构化可检索的 Markdown 素材库/土壤库。当用户有一个塞满文件的素材包/资料包/合集文件夹,想"整理成知识库""清洗成可检索的库""做成写作土壤""提取里面的文字内容""归档这些资料"时使用,即使没明说"清洗"二字。也适用于:几十上百个 PDF/PPT 要批量提取文字、扫描版 PDF 要 OCR、特殊字体 PDF 提取出乱码要修、老格式 .ppt 打不开要转换、资料整理完想删原始大文件省空间。处理的是"一批文件"而非单个文档(单文件转换用 pdf/docx/pptx 等专用 skill)。
---

# 文档素材包清洗归档

把一个混杂的文档文件夹变成结构化、可 grep/read 检索的 Markdown 库,给写作 Harness / agent 当素材土壤,或给人浏览。
...
```
打印类型分布,然后提取所有 PDF/PPTX/DOCX/DOC/epub 到 `<库>/_raw/all/*.txt`,写 `manifest.json`。每个文件判 `quality`:`text`(文字型,可直接用)/`sparse`(稀疏)/`image`(图片型,内容在图里)/`needs_conversion`(老格式ppt)。按文件名关键词自动分 `category`。
看输出的质量分布,决定后面几步做不做。
...
```
LibreOffice 把 `needs_conversion` 的老 .ppt 转 pptx 再提取。逐个转(防一个损坏带累整批),损坏的标 failed。

### 5. 扫描书 OCR(如果图片型里有文字书)
```bash
python3 scripts/ocr_books.py --src "<素材包>" --out "<库目录>"
```
先对每个图片型 PDF 采样几页 OCR,按识别字数区分**扫描文字书**(值得全本 OCR)和**纯设计版面**(跳过)。再全本 OCR 扫描书。
注意:`--threshold` 默认 150 字/页。设计型文案合集(艺术字)会被判低于阈值跳过,那类要视觉模型读(见下方"图片型的取舍")。

### 6. 分类归档 + 建索引(必做,每次改完数据都重跑)
```bash
python3 scripts/build.py --out "<库目录>"
```
清空类目目录,由 manifest+_raw 重新生成每文件的标准化 MD,大类按关键词分子类,生成 `INDEX.md` 和 `提取质量报告.md`。
默认只为有真实内容的 text/sparse 生成 MD;想保留图片型占位加 `--keep-image`。
这步幂等,是"重建"工具——前面任何阶段改了 manifest 都重跑它刷新库。

### 7. 质量校对(删原始前必做)
```bash
python3 scripts/verify.py --out "<库目录>"
```
检测空/过短、乱码残留、OCR 字数偏低。有异常先回到对应阶段修(乱码→3,空/OCR偏低→5或单独OCR),再校对,直到文字类无异常。
