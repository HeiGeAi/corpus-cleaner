...
def main():
    ...
    src_root = os.path.realpath(os.path.abspath(a.src))
    if os.path.dirname(src_root) == src_root:
    ...
    try:
        ...
        for r in manifest:
            safe_join(a.src, source_rels[id(r)])
            if id(r) in archive_rels:
                safe_join(a.out, archive_rels[id(r)])
    ...
    for r in manifest:
        try:
            source_stats[id(r)] = secure_file_stat(a.src, source_rels[id(r)])
        ...
        if q in ("text", "sparse"):
            ...
            try:
                archive_size = secure_file_size(a.out, archive_rel) if archive_rel else -1
            except FileNotFoundError:
                archive_size = -1
            if archive_size > 10:
                to_delete.append(r)
    ...
    for r in to_delete:
        ...
        if r.get("quality") in ("text", "sparse"):
            if not archive_rel or secure_file_size(a.out, archive_rel) <= 10:
    ...
    try:
        leftover = [f for f in secure_listdir(a.src) if not f.startswith(".")]
    except FileNotFoundError:
        leftover = []
