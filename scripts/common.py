...
def _open_root_fd(root, create=False):
    ...
    if isinstance(root, PinnedRoot) and root.snapshot is None:
    ...
    try:
        for part in parts:
            ...
            os.close(fd)
            fd = child
        if isinstance(root, PinnedRoot) and not _same_object(root.snapshot, os.fstat(fd)):
            raise RuntimeError("根目录在预检后被替换")
        return fd
    except Exception:
        os.close(fd)
        raise
...
def pin_root(root, missing_ok=False):
    """解析一次根目录，并固定其设备号与 inode。"""
    path = os.path.realpath(os.path.abspath(os.fspath(root)))
    try:
        fd = _open_root_fd(path)
    except FileNotFoundError:
        if missing_ok:
            return PinnedRoot(path, None)
        raise
    try:
        return PinnedRoot(path, os.fstat(fd))
    finally:
        os.close(fd)
